...
export default function Catalogus() {
  ...
  // Filter fruit products (V09 prefix)
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      const code = product.artikelcode || '';
      return code.startsWith('V09');
    });
  }, [products]);
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="content-wrapper">
        ...
        {!showSkeletons && (
          <div className="results-info-container">
            <div className="results-info">
              <h1 className="text-2xl font-bold text-gray-900 mb-2">Catalogus</h1>
              {searchTerm ? (
                <span>
                  {filteredProducts.length > 0 
                    ? `${filteredProducts.length} ${t('products.found')} voor "${searchTerm}"` 
                    : 'Geen resultaten gevonden'}
                </span>
              ) : (
                <span>
                  {filteredProducts.length > 0 
                    ? `${filteredProducts.length} fruit producten beschikbaar` 
        ...
        {/* Empty state */}
        {!showSkeletons && !error && filteredProducts.length === 0 && (
          <div className="no-results">
